...
class data():
    def __init__(self, lambda_s_a=1.0, lambda_s_b=0.1, industry_dim=38, sample_threshold=5,
    ...
    def read_data_from_file(self):
        ...
        with open('./source_file/raw_finance_data.p', 'rb') as f:
            self.raw_data = pickle.load(f)  ## it is a list, every row is [i,j,m,sl,su]
        with open('./source_file/the dict of W_in finance.p', 'rb') as f:
            self.W_in = pickle.load(f)  ## it is a dict
        with open('./source_file/the dict of voc_length finance.p', 'rb') as f:
            self.voc_length = pickle.load(f)  ## it is a dict
        with open('./source_file/topic_distribution_dict_5_finance.p', 'rb') as f:
            self.varphi = pickle.load(f)
        with open('./source_file/topic_words_5_finance.p', 'rb') as f:
            self.phi = pickle.load(f)
        with open('./source_file/side_feature_finance.p', 'rb') as f:
            self.side_feature = pickle.load(f)
        with open('./source_file/words_count_finance.p', 'rb') as f:
    ...
    def generate_fold(self, folds=5, lower_or_upper=0, shuffle=True, seed=100, construct_baseline_file=False):
        ...
        while fold < folds:
            ...
            with open('./source_file/inner2raw_i.p', 'wb') as f:
                pickle.dump(inner2raw_i, f)
    ...
    def generate_proportional_data(self, train_proportion=0.9, lower_or_upper=0,
        ...
        if construct_baseline_file:
            if self.lower_or_upper == 0:
                ...
                name = 'upper'
            with open('train_data_finance_' + name + '_trainpropotion{}.txt'.format(train_proportion),
                      'w') as f:
                for i, jm, s in self.train_set:
                    f.write(str(i) + '\t' + str(jm) + '\t' + str(s) + '\n')

            with open('test_data_finance_' + name + '_trainpropotion{}.txt'.format(train_proportion),
                      'w') as f:
    ...
    def generate_new_position_data(self, train_proportion=0.9, lower_or_upper=0, seed=100):
        ...
        for i, j, m, sl, su, in self.raw_data:
            if i in select_set:
                self.raw_test.append((int(i), int(j), int(m), sl, su))
            else:
                self.raw_train.append((int(i), int(j), int(m), sl, su))
        ...
        self.construct_inner_params()
        self.construct_salary_matrix(self.train_set)
        self.construct_test_set(self.raw_test, include_new_position=True)
        self.construct_side_feature_matrix(self.raw_train)
        self.construct_similarity_matrix()
        self.construct_reduced_T_for_tadw()
        # print('num_train:{},num_test:{}'.format(len(self.train_set), len(self.test_set)))
    ...
    def construct_salary_matrix(self, train_set):
        ...
        self.X = np.zeros([self.num_position, self.M])+0.00
        ...
        self.S_matrix[np.where(self.X == 0.00)] = 0
    ...
    def construct_baseline_file(self, fold):
        if self.lower_or_upper == 0:
            ...
            name = 'upper'
        with open('train_data_finance_' + name + '_fold{}.txt'.format(fold), 'w') as f:
            for i, jm, s in self.train_set:
                f.write(str(i) + '\t' + str(jm) + '\t' + str(s) + '\n')

        with open('test_data_finance_' + name + '_fold{}.txt'.format(fold), 'w') as f:
            for i, jm, s in self.test_set:
                f.write(str(i) + '\t' + str(jm) + '\t' + str(s) + '\n')

    def construct_side_feature_matrix(self, raw_train):
        self.Y = [np.zeros([self.M, 5]),
                  np.zeros([self.M, 6]),
                  np.zeros([self.M, 11]),
                  ]
    ...
    def construct_similarity_matrix(self):
        ...
        with open('./source_file/auxiliary_position_finance.p', 'rb') as f:
            auxiliary_position = pickle.load(f)
        with open('./source_file/auxiliary_company_finance.p', 'rb') as f:
    ...
    def construct_reduced_T_for_tadw(self):
        with open('./source_file/reduced_T_for_tadw_finance.p', 'rb') as f:
            T=pickle.load(f)
