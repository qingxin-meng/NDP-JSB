...
class optimization():
    def __init__(self, beta=1, omega=1, max_voc=5000, lambda_t=1,
                 ...
                 K=20, industry_dim=38, radius=1,  update_varphi=True):  # indury_dim 38
    ...
    def initialization(self):
        ...
        self.delta.append(np.ones([11, self.K]) / 11.0)
    ...
    def train_predict(self, input, train_epoch=None):
        ...
        if train_epoch is None:
            ...
            for epoch in range(50):
                ...
                test_rmse, test_mae,test_pr,test_pv, for_result_analysis = self.predict(self.test_set, output_predictions=True)
    ...
    def compute_loss(self):
        ...
        self.Expectation_of_log_delta_3D = [
                                            np.zeros([5, self.K]),
                                            np.zeros([6, self.K]),
                                            np.zeros([11, self.K])]  # [D=5,dim_d, K]
        for d in range(3):
    ...
    def update_theta(self):
        ...
        time1 = time.time()
        mins = (time1 - self.s_time) / 60
        # print('updating theta need time {:.2f}\n'.format(mins))
    ...
    def predict(self, test_data, output_predictions=False):
        ...
        for i, jm, s in test_data:
            ...
            if isinstance(i, list):
                s_predict += np.sum(i * (self.mu_ @ self.Z[jm, :]))
            s_predict = s_predict+ self.b_position[i] + self.b_company[jm]
            if s_predict > self.max:
                s_predict = self.max
            if s_predict < self.min:
                s_predict = self.min
            rmse += (s_predict - s) ** 2
